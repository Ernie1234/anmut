...
function TeamCard({ avatar, name, job, quote, url }) {
  return (
    <div className="max-w-sm bg-slate-900 rounded-sm md:rounded-md lg:rounded-lg p-5 md:pt-8 lg:pt-8">
      <div className="flex flex-col items-center">
        <img
          ...
          height="512"
        />
        <h5 className="text-xl font-medium text-amber-300">{name}</h5>
        <span className="text-sm text-gray-500">{job}</span>
        <h5 className="mt-3 text-sm text-slate-500">{quote}</h5>
        <div className="mt-4 flex space-x-3 lg:mt-6">
          <a
            href={url}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center rounded-lg bg-amber-300 px-4 py-2 text-center text-sm font-medium text-slate-800 hover:bg-amber-500 focus:outline-none focus:ring-4 focus:ring-amber-300 gap-1.5"
          >
            <span>
              <FaFacebook size={25} />
            </span>
